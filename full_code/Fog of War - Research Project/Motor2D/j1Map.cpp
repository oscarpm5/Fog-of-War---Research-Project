...
void j1Map::Draw()
{
	...
	while (i<data.layers.size())
	{
		...
		if (data.layers[i]->properties.Get("Nodraw") != 0)
		{
			i++;
			continue;
		}
...
bool j1Map::Load(const char* file_name)
{
	...
	if(ret == true)
	{
		...
		{
			LOG("Tileset ----");
			LOG("name: %s firstgid: %d", data.tilesets[i]->name.GetString(), data.tilesets[i]->firstgid);
			LOG("tile width: %d tile height: %d", data.tilesets[i]->tile_width, data.tilesets[i]->tile_height);
			LOG("spacing: %d margin: %d", data.tilesets[i]->spacing, data.tilesets[i]->margin);
		}
		...
		for (int i = 0; i < data.layers.size(); i++)
		{
			...
			LOG("tile width: %d tile height: %d", data.layers[i]->width, data.layers[i]->height);
		}
	}
